...
class SynergyServer:
	...
	async def on_new_connection(self, websocket, path):
		if path == '/client':
			print('New Client connection | {websocket.remote_address}')
			aid = None
			client = None
			try:
				while True:
...
class SynergyClient:
	...
	def get_rooms(self) -> List[str]:
		...
		task = asyncio.async(self.sleep(0.5))
	...
	async def async_send_dict(self, x: dict) -> None:
		self.socket.send(dumps(x))
		return

	def sync_send_dict(self, x: dict) -> None:
		#task = self.async_send_dict(x)
		task = asyncio.async(self.async_send_dict(x))
		asyncio.get_event_loop().create_task(task)
		return
